...
local speedMin, speedMax = 10, 25
local flySpeed = math.clamp(cfgFarmSpeed, speedMin, speedMax)
...
Size = ud2((flySpeed - speedMin) / (speedMax - speedMin), 0, 1, 0),
...
Size = ud2(0,18,0,18),
Position = ud2((flySpeed - speedMin) / (speedMax - speedMin), -9, 0.5, -9),
...
flySpeed = math.clamp(math.floor(value + 0.5), speedMin, speedMax)
speedValueLabel.Text = tostring(flySpeed)
speedInput.Text = tostring(flySpeed)
local ratio = (flySpeed - speedMin) / (speedMax - speedMin)
sliderFill.Size = ud2(ratio, 0, 1, 0)
sliderKnob.Position = ud2(ratio, -9, 0.5, -9)
...
setFlySpeed(speedMin + ratio * (speedMax - speedMin))
end
                ...
                if coin and coin.Parent and coin:IsDescendantOf(workspace) then
                    ...
                    if coinPart then
                        ...
                        if isFarmActive and coin and coin.Parent then
                            ...
                            if cfgDieAfterBag and collectedCoins >= 40 then
