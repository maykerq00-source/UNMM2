...
--  ESP Fixed: TRULY Instant Role Detection & Fixed Coin Farm
-- ============================================================
...
local cfgAntiKill        = savedCfg.antiKill or false
local cfgAntiKillRadius  = savedCfg.antiKillRadius or 30
...
antiKill      = cfgAntiKill,
antiKillRadius = cfgAntiKillRadius
})
end
...
local tabs        = {}
local tabSections = {Home = {}, ESP = {}, Farm = {}}
...
local function getActualRole(player)
    -- 1. Scan Character and Backpack for actual items (Instant)
...
-- Fetch role data on a cooldown using GetPlayerData remote (INSTANT detection fallback)
task.spawn(function()
...
-- Farm Speed Dial Card
local speedCard = Make("Frame", {
Size = ud2(1,-8,0,90), BackgroundColor3 = C.part_bg,
BackgroundTransparency = 0.3, BorderSizePixel = 0,
...
Make("TextLabel", {
Size = ud2(1,-28,0,20), Position = ud2(0,14,0,10),
...
Size = ud2(0,60,0,20), Position = ud2(1,-86,0,10),
BackgroundTransparency = 1, Text = tostring(flySpeed),
TextColor3 = C.accent, TextSize = 13, Font = bold,
TextXAlignment = Enum.TextXAlignment.Right, Parent = speedCard,
})

local sliderTrack = Make("Frame", {
Size = ud2(1,-28,0,20), Position = ud2(0,14,0,40),
BackgroundColor3 = C.surface, BackgroundTransparency = 0.55,
...
Size = ud2(0,18,0,18),
Position = ud2((flySpeed - speedMin) / (getSpeedMax() - speedMin), 0, 0.5, -9),
AnchorPoint = Vector2.new(0.5, 0.5),
...
Size = ud2(1,0,0,28), Position = ud2(0,0,0.5,-14),
AnchorPoint = Vector2.new(0,0),
BackgroundTransparency = 1, Text = "",
ZIndex = 2, Parent = sliderTrack,
})

local speedInput = Make("TextBox", {
Size = ud2(0,78,0,24), Position = ud2(0,14,0,66),
BackgroundColor3 = C.surface, BackgroundTransparency = 0.3,
Text = tostring(flySpeed), TextColor3 = C.textPri,
Font = semi, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Center,
ClearTextOnFocus = false, Parent = speedCard,
})
Make("UICorner", {CornerRadius = ud(1,0), Parent = speedInput})
...
sliderKnob.Position = ud2(ratio, 0, 0.5, -9)
...
local ratio = localX / trackWidth
setFlySpeed(speedMin + ratio * (getSpeedMax() - speedMin))
end
...
-- Anti-Kill Section (collapsible)
local antiKillSection = makeSection("Anti-Kill", farmContainer, "Farm", false, false)

partState["antiKill"] = cfgAntiKill
createTogglePart(antiKillSection.Content, "Anti-Kill", "antiKill", 1, false, function(state)
cfgAntiKill = state
triggerAutoSave()
if state then
startAntiKill()
...
Size = ud2(1,-8,0,80), BackgroundColor3 = C.part_bg,
BackgroundTransparency = 0.3, BorderSizePixel = 0,
LayoutOrder = 2, Parent = antiKillSection.Content,
})
...
Make("TextLabel", {
Size = ud2(1,-28,0,20), Position = ud2(0,14,0,10),
BackgroundTransparency = 1, Text = "Detection Radius",
TextColor3 = C.textPri, TextSize = 13, Font = semi,
TextXAlignment = Enum.TextXAlignment.Left, Parent = radiusCard,
})

local radiusMin, radiusMax = 10, 80
local detectRadius = math.clamp(cfgAntiKillRadius, radiusMin, radiusMax)

local radiusValueLabel = Make("TextLabel", {
Size = ud2(0,60,0,20), Position = ud2(1,-86,0,10),
...
local radiusTrack = Make("Frame", {
Size = ud2(1,-28,0,20), Position = ud2(0,14,0,40),
BackgroundColor3 = C.surface, BackgroundTransparency = 0.55,
BorderSizePixel = 0, Parent = radiusCard,
})
Make("UICorner", {CornerRadius = ud(1,0), Parent = radiusTrack})

local radiusFill = Make("Frame", {
Size = ud2((detectRadius - radiusMin) / (radiusMax - radiusMin), 0, 1, 0),
BackgroundColor3 = C.accent, BorderSizePixel = 0, Parent = radiusTrack,
})
Make("UICorner", {CornerRadius = ud(1,0), Parent = radiusFill})

local radiusKnob = Make("TextButton", {
Size = ud2(0,18,0,18),
Position = ud2((detectRadius - radiusMin) / (radiusMax - radiusMin), 0, 0.5, -9),
AnchorPoint = Vector2.new(0.5, 0.5),
BackgroundColor3 = C.knob, BorderSizePixel = 0, Text = "",
ZIndex = 3, Parent = radiusTrack,
})
Make("UICorner", {CornerRadius = ud(1,0), Parent = radiusKnob})
Make("UIStroke", {Color = rgb(0,0,0), Thickness = 1, Transparency = 0.7, Parent = radiusKnob})

local radiusHitbox = Make("TextButton", {
Size = ud2(1,0,0,28), Position = ud2(0,0,0.5,-14),
BackgroundTransparency = 1, Text = "",
ZIndex = 2, Parent = radiusTrack,
})
...
radiusValueLabel.Text = tostring(detectRadius)
local ratio = (detectRadius - radiusMin) / (radiusMax - radiusMin)
radiusFill.Size = ud2(ratio, 0, 1, 0)
radiusKnob.Position = ud2(ratio, 0, 0.5, -9)
cfgAntiKillRadius = detectRadius
if not skipSave then triggerAutoSave() end
end

local function refreshRadiusPosition(x)
local trackWidth = radiusTrack.AbsoluteSize.X
if trackWidth <= 0 then return end
local localX = math.clamp(x - radiusTrack.AbsolutePosition.X, 0, trackWidth)
local ratio = localX / trackWidth
setDetectRadius(radiusMin + ratio * (radiusMax - radiusMin))
end
...
radiusHitbox.InputBegan:Connect(function(input)
if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
radiusDragging = true
...
-- Exceed Speed Limit toggle (moved inside Anti-Kill section)
partState["exceedSpeed"] = cfgExceedSpeed
createTogglePart(antiKillSection.Content, "Exceed Speed Limit", "exceedSpeed", 3, false, function(state)
...
-- ============================================================
-- FIXED FARM LOGIC - Un-Anchoring Issue + Collection Hitboxes
-- ============================================================
...
-- noclip while farming so the character can actually reach/overlap the coin
-- (this is what lets the game's own Touched event fire and register the pickup)
...
local function findClosestCoin()
    ...
    -- Ensure we check where coins actually spawn first
    ...
    for _, obj in ipairs(searchArea) do
        if obj.Name == "Coin_Server" and not visitedPositions[obj] then
            -- We must target the actual BasePart with the hitbox
                ...
                if coin and coin.Parent and coin:IsDescendantOf(workspace) then
                    ...
                    if coinPart then
                        ...
                        if isFarmActive and coin and coin.Parent then
                            -- backup: force a touch in case the engine didn't register the overlap
...
-- Anti-Kill: auto-invis when near murderer or thrown knife
local antiKillEnabled = false
local antiKillConn = nil
local antiKillParts = {}

local function setupAntiKillParts()
antiKillParts = {}
local char = LocalPlayer.Character
if char then
for _, v in pairs(char:GetDescendants()) do
if v:IsA("BasePart") then
table.insert(antiKillParts, v)
end
end
end
end

local function applyAntiKillTransparency(transparent)
for _, v in pairs(antiKillParts) do
if v.Parent then
v.Transparency = transparent and 0.5 or 0
end
end
end

local function isKnifeNearby(hrpPosition)
for _, obj in pairs(workspace:GetDescendants()) do
if obj:IsA("BasePart") and obj.Name:find("[Kk]nife") then
local dist = (obj.Position - hrpPosition).Magnitude
if dist < 15 then
return true
end
end
...
if dist < cfgAntiKillRadius then
hasInvis = true
break
end
end
...
if not hasInvis then
hasInvis = isKnifeNearby(hrp.Position)
end
if hasInvis then
local _CFrame = hrp.CFrame
local _Offset = hum.CameraOffset
local farPos = _CFrame * CFrame.new(0, -200000, 0)
local objOffset = farPos:ToObjectSpace(CFrame.new(_CFrame.Position)).Position
hrp.CFrame = farPos
hum.CameraOffset = objOffset
applyAntiKillTransparency(true)
RunService.RenderStepped:Wait()
if hrp and hum and hrp.Parent and hum.Health > 0 then
hrp.CFrame = _CFrame
hum.CameraOffset = _Offset
applyAntiKillTransparency(false)
end
end
end)
end
...
if cfgAntiKill then task.defer(startAntiKill) end
...
print("[UNMM2] Loaded Successfully - Instant Role Detection via GetPlayerData")